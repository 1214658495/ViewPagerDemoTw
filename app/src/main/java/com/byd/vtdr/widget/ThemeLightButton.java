...
//		int res = mTheme.get(theme);
		//if(res > 0) {
			...
			if(!isEnabled()){
				...
				Log.d(TAG, "*********setTextColor(R.color.service_item_confirm_color***********");
			}else{
				if(theme == Theme.NORMAL){
					...
					Log.d(TAG, "***getResources().getColorStateList(R.color.press_selector_text_sport)***");
				}		 					
			}
			setTextColor(colorStateList);//设置文本的颜色
		    Log.d(TAG, "**********setTextColor(colorStateList)********");
	}
